...
// Client wraps an HTTP client with Enamad-specific configuration.
type Client struct {
	httpClient *http.Client
	config     *client.HeaderConfig
}

// NewClient creates a new Enamad HTTP client.
func NewClient(cfg *config.Config) *Client {
	return &Client{
		httpClient: client.NewHTTPClient(&cfg.HTTP),
		config: &client.HeaderConfig{
			Origin:  cfg.Services.Enamad.BaseURL,
			BaseURL: cfg.Services.Enamad.BaseURL,
		},
	}
}
